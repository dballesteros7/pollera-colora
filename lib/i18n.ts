...
const M: Record<string, Msg> = {
  ...
  "recap.buddyLine": { es: "Piensan igualito: coinciden en {n} marcadores exactos (en todas las pollas).", en: "Two minds, one scoreline: you match on {n} exact predictions (across all pollas).", de: "Ein Herz und eine Seele: {n} identische Tipps (über alle Pollas).", it: "Stessa testa: {n} pronostici identici (in tutte le polle).", pt: "Pensam igual: {n} palpites idênticos (em todas as pollas).", fr: "Mêmes idées : {n} pronostics identiques (toutes pollas confondues).", zh: "想法一致：你们有 {n} 个完全相同的比分预测（跨所有竞猜）。", zht: "想法一致：你們有 {n} 個完全相同的比分預測（跨所有競猜）。", nl: "Twee handen op één buik: {n} identieke voorspellingen (over alle polla's)." },
