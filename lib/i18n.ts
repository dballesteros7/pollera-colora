...
const M: Record<string, Msg> = {
  ...
  "recap.buddyLine": { es: "Piensan igualito: coinciden en {n} de {total} pronósticos (en todas las pollas).", en: "Two minds, one scoreline: you match on {n} of {total} predictions (across all pollas).", de: "Ein Herz und eine Seele: {n} von {total} Tipps identisch (über alle Pollas).", it: "Stessa testa: {n} su {total} pronostici identici (in tutte le polle).", pt: "Pensam igual: {n} de {total} palpites idênticos (em todas as pollas).", fr: "Mêmes idées : {n} pronostics identiques sur {total} (toutes pollas confondues).", zh: "想法一致：在 {total} 个共同预测中你们有 {n} 个完全相同（跨所有竞猜）。", zht: "想法一致：在 {total} 個共同預測中你們有 {n} 個完全相同（跨所有競猜）。", nl: "Twee handen op één buik: {n} van {total} voorspellingen identiek (over alle polla's)." },
